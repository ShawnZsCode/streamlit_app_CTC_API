...
class ToolManager:
    ...
    def register_from_openai_schema(self, schema: Dict[str, Any], implementation: callable) -> None:
        """Register a tool from OpenAI function schema format"""
        function_def = schema.get("function", schema)  # Handle both wrapped and unwrapped schemas
    ...
    def register_tools_from_schemas(self, tools_config: List[Dict[str, Any]], implementations: Dict[str, Callable]) -> None:
        ...
        for tool_schema in tools_config:
            ...
            if function_name in implementations:
                self.register_from_openai_schema(tool_schema, implementations[function_name])
            else:
        ...
        try:
            async with session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    chat_memory.context_data['active_project_last_updated'] = datetime.now()
...
async def get_project_levels() -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Error fetching levels: {str(e)}"
            }
...
async def create_floor_plan(Name: str, LevelId: int, ViewTemplateId: int, ScopeBoxId: int = 0) -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            ...
            return {
                "success": False,
                "error": f"Error creating floor plan: {str(e)}"
            }
